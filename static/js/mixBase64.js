...
function initKSA(passwd) {
  ...
  if (typeof passwd === 'string') {
    ...
    key = CryptoJS.SHA256(passwd);
    const arrayBuffer = new Uint8Array(key.words.length * 4);
    for (let i = 0; i < key.words.length; i++) {
        arrayBuffer[i * 4 + 0] = (key.words[i] & 0xff000000) >>> 24;
        arrayBuffer[i * 4 + 1] = (key.words[i] & 0x00ff0000) >>> 16;
        arrayBuffer[i * 4 + 2] = (key.words[i] & 0x0000ff00) >>> 8;
        arrayBuffer[i * 4 + 3] = (key.words[i] & 0x000000ff);
    }
    key = arrayBuffer;
  }
  const K = []
...
function MixBase64(passwd, salt = 'mix64') {
  ...
  this.encode = function (bufferOrStr, encoding = 'utf-8') {
    ...
    const buffer = bufferOrStr instanceof Buffer ? bufferOrStr : new TextEncoder(encoding).encode(str)
    console.log(buffer);
    let result = ''
    let arr = []
    let bt = []
  ...
  this.decode = function (base64Str) {
    ...
    while (i < base64Str.length) {
        ...
        writeUInt8(buffer,(enc1 << 2) | (enc2 >> 4), j++)
        if (enc3 !== 64) {
          writeUInt8(buffer,((enc2 & 15) << 4) | (enc3 >> 2), j++)
        }
        if (enc4 !== 64) {
          writeUInt8(buffer,((enc3 & 3) << 6) | enc4, j++)
        }
    }
      return buffer
    }
...
const bufferArr = encoder.encode(text);
  let count = 0
